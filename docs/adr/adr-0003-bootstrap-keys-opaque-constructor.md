...
**Constructor:** `constructor(int64 _bootstrapAlg,
...
`bootstrapConfig()` (alg + opaque key).

## Context

The previous constructor took an address parameter (removed);
...
- Optional getter `bootstrapConfig()` returns (alg, key) in same form as
  constructor.

...
contract LibCborHelper {
    ...
    function callDecodePaymentClaims(bytes memory d)
        external
        pure
        returns (LibCbor.PaymentClaims memory)
    {
...
contract LibCborTest is Test {
    ...
    /// @notice Plan 0012 §4.
    ///    5: map with extra unknown keys still decodes known claims (forward
    ///    compatibility)
    function test_decodePaymentClaims_extraUnknownKeys_decodesKnownClaims()
        public
        pure
    {
        ...
        bytes memory payload = abi.encodePacked(
            ...
            payer,
            hex"21",
            hex"00", // checkpoint_start 0
            hex"22",
            hex"1864", // checkpoint_end 100
            hex"23",
            hex"00", // max_height 0
            hex"00",
            hex"01" // unknown key 0: 1 (skip)
        );
        LibCbor.PaymentClaims memory claims =
            LibCbor.decodePaymentClaims(payload);
        assertEq(claims.logId, logId);

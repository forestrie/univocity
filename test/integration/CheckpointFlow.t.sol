...
/// @notice Integration tests: full bootstrap, R5 receipt, permissionless submission
contract CheckpointFlowTest is Test, IUnivocityEvents {
    ...
    uint256 internal constant SIGNER_PK = 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80;
    ...
    function test_fullFlow_bootstrapInitializesAndPublishesAuthority() public {
        ...
        (bytes memory receipt, bytes32[] memory inclusionProofPath, bytes32[] memory authorityAcc) =
            _buildReceiptAndAuthorityState(authorityLogId, 0, 10, 0, IDTIMESTAMP_0);

        fresh.publishCheckpoint(authorityLogId, 1, authorityAcc, receipt, new bytes32[][](0), 0, inclusionProofPath, IDTIMESTAMP_0);
    ...
    /// @notice Build a minimal KS256-signed receipt and authority state; leaf = H(idtimestampBe ‖ sha256(receipt)) (ADR-0030).
    function _buildReceiptAndAuthorityState(bytes32 targetLogId, uint64 start, uint64 end, uint64 maxHeight, bytes8 idtimestampBe)
        internal
        view
        returns (bytes memory receipt, bytes32[] memory inclusionProofPath, bytes32[] memory authorityAccumulator)
    {
        bytes memory payload = abi.encodePacked(
            hex"a5",
            hex"025820", targetLogId,
        ...
        authorityAccumulator[0] = sha256(abi.encodePacked(idtimestampBe, sha256(receipt)));

        inclusionProofPath = new bytes32[](0); // leaf index 0, empty path (single-node MMR)
    }
    ...
    /// @notice Full flow: first checkpoint has receipt as first entry; then we add TARGET_LOG receipt to authority and user checkpoints
    function test_fullFlow_userCheckpointsWithReceipt() public {
        // First checkpoint: bootstrap receipt for authority log
        (bytes memory authorityReceipt, bytes32[] memory authInclusionPath, bytes32[] memory authorityAcc) =
            _buildReceiptAndAuthorityState(authorityLogId, 0, 10, 1000, IDTIMESTAMP_0);
        univocity.publishCheckpoint(authorityLogId, 1, authorityAcc, authorityReceipt, new bytes32[][](0), 0, authInclusionPath, IDTIMESTAMP_0);

        // Second checkpoint to authority log: add TARGET_LOG receipt at index 1 (size 2 = two peaks at 0, 1)
        (bytes memory targetReceipt,,) = _buildReceiptAndAuthorityState(TARGET_LOG, 0, 10, 1000, IDTIMESTAMP_1);
        bytes32 authLeaf = sha256(abi.encodePacked(IDTIMESTAMP_0, sha256(authorityReceipt)));
        bytes32 targetLeaf = sha256(abi.encodePacked(IDTIMESTAMP_1, sha256(targetReceipt)));
        ...
        univocity.publishCheckpoint(authorityLogId, 2, accSize2, authorityReceipt, consistencyProof, 0, new bytes32[](0), IDTIMESTAMP_0);

        // User checkpoint: receipt for TARGET_LOG at index 1 (peak; empty inclusion proof)
        ...
        univocity.publishCheckpoint(TARGET_LOG, 1, targetAcc, targetReceipt, new bytes32[][](0), 1, new bytes32[](0), IDTIMESTAMP_1);
    ...
    function test_fullFlow_sameReceiptDifferentSubmitters() public {
        ...
        // Second checkpoint to authority: add TARGET_LOG receipt at index 1 (size 2 = two peaks)
        (bytes memory receipt,,) = _buildReceiptAndAuthorityState(TARGET_LOG, 0, 2, 0, IDTIMESTAMP_1);
        bytes32 authLeaf = sha256(abi.encodePacked(IDTIMESTAMP_0, sha256(authorityReceipt)));
        bytes32 targetLeaf = sha256(abi.encodePacked(IDTIMESTAMP_1, sha256(receipt)));
        bytes32[] memory accSize2 = new bytes32[](2);
        accSize2[0] = authLeaf;
        accSize2[1] = targetLeaf;
        bytes32[][] memory consistencyProof = new bytes32[][](1);
        ...
        // First checkpoint (known vector from consistentRoots_0_to_2); receipt at index 1 (peak), empty proof
        bytes32[] memory acc1 = new bytes32[](1);
        acc1[0] = 0xaf5570f5a1810b7af78caf4bc70a660f0df51e42baf91d4de5b2328de0e83dfc;
        vm.prank(address(0xA11ce));
        univocity.publishCheckpoint(TARGET_LOG, 1, acc1, receipt, new bytes32[][](0), 1, new bytes32[](0), IDTIMESTAMP_1);

        // Second checkpoint size=3 (1 peak; peaks(2).length==1) with consistency proof (same vector)
        ...
        univocity.publishCheckpoint(TARGET_LOG, 3, acc2, receipt, proofs, 1, new bytes32[](0), IDTIMESTAMP_1);

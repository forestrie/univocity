...
contract UnivocityTest is UnivocityTestHelper, IUnivocityEvents {
    ...
    function test_firstPublish_emitsInitialized() public {
        ImutableUnivocity newUnivocity =
            new ImutableUnivocity(ALG_KS256, abi.encodePacked(KS256_SIGNER));
        PublishGrant memory g = _publishGrant(
            AUTHORITY_LOG_ID,
            GRANT_ROOT,
    ...
    function test_publishCheckpoint_revertsOnEmptyConsistencyProofs() public {
        ConsistencyReceipt memory emptyProofs = ConsistencyReceipt({
            protectedHeader: _consistencyProtectedHeader(ALG_KS256, 0),
            signature: hex"",
    ...
    function test_es256RecoveredKey_equals_publicKeyP256() public {
        ...
        consistency = _buildConsistencyReceiptES256(_toAcc(leaf0), es256Pk);
        vm.prank(BOOTSTRAP);
        univocity = new ImutableUnivocity(ALG_ES256, abi.encodePacked(kx, ky));
        univocity.publishCheckpoint(
            consistency, _emptyInclusionProof(), idtimestampBe, g
        );
        assertEq(univocity.rootLogId(), AUTHORITY_LOG_ID);
    ...
    function test_firstCheckpoint_es256Receipt_succeeds() public {
        ...
        ConsistencyReceipt memory consistency =
            _buildConsistencyReceiptES256(_toAcc(leaf0), es256Pk);
        vm.prank(BOOTSTRAP);
        ImutableUnivocity es256Univocity =
            new ImutableUnivocity(ALG_ES256, abi.encodePacked(pubX, pubY));
    ...
    function test_verifyCheckpoint_es256ReceiptOnKs256Log_revertsAlgorithmMismatch()
        public
    {
        ImutableUnivocity ks256Univocity =
            new ImutableUnivocity(ALG_KS256, abi.encodePacked(KS256_SIGNER));
        PublishGrant memory g0 = _publishGrant(
            AUTHORITY_LOG_ID,
            GRANT_ROOT,
        ...
        public
    {
        ...
        bytes memory sigStruct = buildSigStructure(
            _consistencyProtectedHeader(ALG_KS256, 1),
        ...
        ConsistencyReceipt memory invalidReceipt = ConsistencyReceipt({
            protectedHeader: _consistencyProtectedHeader(ALG_KS256, 1),

...
contract UnivocityTest is Test, IUnivocityEvents {
    ...
    {
        return _buildBootstrapReceiptAndAccWithBounds(logId, receiptIdtimestampBe, 0, 10, 0);
    }

    /// @notice Same as above with configurable checkpoint_start, checkpoint_end, max_height (for bounds tests).
        ...
        returns (bytes memory receipt, bytes32[] memory accumulator, bytes32[] memory inclusionProof)
    {
        bytes memory payload = abi.encodePacked(
            ...
            hex"21", _uintCbor(start),
            hex"22", _uintCbor(end),
            hex"23", _uintCbor(maxHeight)
        );
    ...
    /// @notice Plan 0012 §4.2: first checkpoint with size > 1 (receipt at index 0, two-peak accumulator)
    function test_firstCheckpoint_sizeTwo_succeeds() public {
        Univocity fresh = new Univocity(BOOTSTRAP, KS256_SIGNER, bytes32(0), bytes32(0));
        (bytes memory receipt,,) = _buildBootstrapReceiptAndAcc(AUTHORITY_LOG_ID, IDTIMESTAMP_AUTH);
        bytes32 leaf0 = sha256(abi.encodePacked(IDTIMESTAMP_AUTH, sha256(receipt)));
        (bytes memory receipt2,,) = _buildBootstrapReceiptAndAcc(TEST_LOG_ID, IDTIMESTAMP_TEST);
        bytes32 leaf1 = sha256(abi.encodePacked(IDTIMESTAMP_TEST, sha256(receipt2)));
        bytes32[] memory accSize2 = new bytes32[](2);
        accSize2[0] = leaf0;
        accSize2[1] = leaf1;
        bytes32[] memory inclusionProof = new bytes32[](0);
        fresh.publishCheckpoint(AUTHORITY_LOG_ID, 2, accSize2, receipt, new bytes32[][](0), 0, inclusionProof, IDTIMESTAMP_AUTH);
        assertEq(fresh.authorityLogId(), AUTHORITY_LOG_ID);
        assertEq(fresh.getLogState(AUTHORITY_LOG_ID).size, 2);
    }

    /// @notice Plan 0012 §4.2: authority log path does not require inclusion proof (second checkpoint to authority)
    function test_publishCheckpoint_authorityLogSecondCheckpoint_noInclusionProofRequired() public {
        (bytes memory receipt,,) = _buildBootstrapReceiptAndAcc(AUTHORITY_LOG_ID, IDTIMESTAMP_AUTH);
    ...
    /// @notice Plan 0012 §4.4: after first checkpoint, authority accumulator peak equals H(idtimestampBe ‖ sha256(receipt))
    function test_firstCheckpoint_authorityFirstLeafMatchesAdr0030Formula() public {
        Univocity fresh = new Univocity(BOOTSTRAP, KS256_SIGNER, bytes32(0), bytes32(0));
        (bytes memory receipt, bytes32[] memory acc,) = _buildBootstrapReceiptAndAcc(AUTHORITY_LOG_ID, IDTIMESTAMP_AUTH);
        bytes32 expectedLeaf = sha256(abi.encodePacked(IDTIMESTAMP_AUTH, sha256(receipt)));
        fresh.publishCheckpoint(AUTHORITY_LOG_ID, 1, acc, receipt, new bytes32[][](0), 0, new bytes32[](0), IDTIMESTAMP_AUTH);
        bytes32[] memory stored = fresh.getLogState(AUTHORITY_LOG_ID).accumulator;
    ...
    function test_publishCheckpoint_revertsWhenCheckpointCountAtOrAboveReceiptEnd() public {
        // Use a fresh contract and a receipt with end=1 so second publish reverts CheckpointCountExceeded(1, 1).
        Univocity fresh = new Univocity(BOOTSTRAP, KS256_SIGNER, bytes32(0), bytes32(0));
        bytes32 logId = keccak256("other-target");
        (bytes memory bootstrapReceipt, bytes32[] memory acc1,) = _buildBootstrapReceiptAndAcc(AUTHORITY_LOG_ID, IDTIMESTAMP_AUTH);
        fresh.publishCheckpoint(AUTHORITY_LOG_ID, 1, acc1, bootstrapReceipt, new bytes32[][](0), 0, new bytes32[](0), IDTIMESTAMP_AUTH);

        (bytes memory receiptEnd1,,) = _buildBootstrapReceiptAndAccWithBounds(logId, IDTIMESTAMP_TEST, 0, 1, 0);
        bytes32 leaf0 = sha256(abi.encodePacked(IDTIMESTAMP_AUTH, sha256(bootstrapReceipt)));
        bytes32 leaf1 = sha256(abi.encodePacked(IDTIMESTAMP_TEST, sha256(receiptEnd1)));
        bytes32[] memory acc2 = new bytes32[](2);
        acc2[0] = leaf0;
        acc2[1] = leaf1;
        bytes32[][] memory consistencyProof = new bytes32[][](1);
        consistencyProof[0] = new bytes32[](0);
        fresh.publishCheckpoint(AUTHORITY_LOG_ID, 2, acc2, bootstrapReceipt, consistencyProof, 0, new bytes32[](0), IDTIMESTAMP_AUTH);

        bytes32[] memory targetAcc = new bytes32[](1);
        targetAcc[0] = 0xaf5570f5a1810b7af78caf4bc70a660f0df51e42baf91d4de5b2328de0e83dfc;
        fresh.publishCheckpoint(logId, 1, targetAcc, receiptEnd1, new bytes32[][](0), 1, new bytes32[](0), IDTIMESTAMP_TEST);
        assertEq(fresh.getLogState(logId).checkpointCount, 1);

        bytes32[] memory accSize3 = new bytes32[](1);
        accSize3[0] = 0xad104051c516812ea5874ca3ff06d0258303623d04307c41ec80a7a18b332ef8;
        bytes32[][] memory proofs = new bytes32[][](1);
        proofs[0] = new bytes32[](1);
        proofs[0][0] = 0xcd2662154e6d76b2b2b92e70c0cac3ccf534f9b74eb5b89819ec509083d00a50;
        vm.expectRevert(abi.encodeWithSelector(IUnivocityErrors.CheckpointCountExceeded.selector, uint64(1), uint64(1)));
        fresh.publishCheckpoint(logId, 3, accSize3, receiptEnd1, proofs, 1, new bytes32[](0), IDTIMESTAMP_TEST);
    }

    function test_publishCheckpoint_revertsWhenSizeExceedsReceiptMaxHeight() public {
        // Receipt: maxHeight = 1; try to publish size 2 (TEST_LOG already at size 1 from setUp)
        (bytes memory receipt,,) = _buildBootstrapReceiptAndAccWithBounds(TEST_LOG_ID, IDTIMESTAMP_TEST, 0, 10, 1);
        bytes32[] memory accSize2 = new bytes32[](2);
        accSize2[0] = keccak256("p0");
        accSize2[1] = keccak256("p1");
        bytes32[][] memory consistencyProof = new bytes32[][](1);
        consistencyProof[0] = new bytes32[](0); // one peak at size 1 → proof for peak 0

        vm.expectRevert(abi.encodeWithSelector(IUnivocityErrors.MaxHeightExceeded.selector, uint64(2), uint64(1)));
        univocity.publishCheckpoint(TEST_LOG_ID, 2, accSize2, receipt, consistencyProof, 1, new bytes32[](0), IDTIMESTAMP_TEST);
    }

    function test_publishCheckpoint_revertsWhenReceiptTargetsDifferentLog() public {
        // Regular log: publish to TEST_LOG_ID but receipt has targetLogId = other log
        bytes32 otherLogId = keccak256("other-log");
        (bytes memory receipt,,) = _buildBootstrapReceiptAndAccWithBounds(otherLogId, IDTIMESTAMP_TEST, 0, 10, 0);
        bytes32[] memory accumulator = new bytes32[](1);
        accumulator[0] = keccak256("peak1");

        vm.expectRevert(
            abi.encodeWithSelector(IUnivocityErrors.ReceiptLogIdMismatch.selector, TEST_LOG_ID, otherLogId)
        );
        univocity.publishCheckpoint(TEST_LOG_ID, 1, accumulator, receipt, new bytes32[][](0), 1, new bytes32[](0), IDTIMESTAMP_TEST);
    }
    ...
    // IUnivocityErrors coverage: FirstCheckpointSizeTooSmall, BootstrapReceiptMustBeFirstEntry,
    // OnlyBootstrapAuthority, ReceiptLogIdMismatch, InvalidReceiptInclusionProof → see test_firstCheckpoint_* and test_publishCheckpoint_*.
    // SizeMustIncrease, InvalidAccumulatorLength, InvalidConsistencyProof → test_publishCheckpoint_revertsOn*.
    // CheckpointCountExceeded, MaxHeightExceeded, ReceiptLogIdMismatch (regular log) → test_publishCheckpoint_revertsWhen*.
    // AlreadyInitialized: only in _initializeAuthorityLog when authorityLogId != 0; not reachable from publishCheckpoint (first-checkpoint block only runs when authorityLogId == 0).
    // NotInitialized, LogNotFound, InvalidSignatureChain: not used in Univocity.sol.
    function test_errorCoverageMatrix_allReachableErrorsHaveExplicitRevertTest() public pure {
        // Ensure each reachable error has a non-zero selector (matrix documented in comments above)
        assertTrue(uint32(bytes4(IUnivocityErrors.FirstCheckpointSizeTooSmall.selector)) != 0);
        assertTrue(uint32(bytes4(IUnivocityErrors.OnlyBootstrapAuthority.selector)) != 0);
        assertTrue(uint32(bytes4(IUnivocityErrors.InvalidConsistencyProof.selector)) != 0);
        assertTrue(uint32(bytes4(IUnivocityErrors.CheckpointCountExceeded.selector)) != 0);
        assertTrue(uint32(bytes4(IUnivocityErrors.MaxHeightExceeded.selector)) != 0);
        assertTrue(uint32(bytes4(IUnivocityErrors.ReceiptLogIdMismatch.selector)) != 0);
    }

    // === Plan 0012 Phase C: ES256 receipt (4.5 item 12) ===
    /// @notice First checkpoint with ES256-signed receipt; Univocity deployed with es256X/Y only.
    function test_firstCheckpoint_es256Receipt_succeeds() public {
        uint256 es256Pk = 1;
        (uint256 pubX, uint256 pubY) = vm.publicKeyP256(es256Pk);
        vm.prank(BOOTSTRAP);
        Univocity es256Univocity = new Univocity(BOOTSTRAP, address(0), bytes32(pubX), bytes32(pubY));

        bytes8 idtimestampBe = bytes8(0);
        (bytes memory receipt, bytes32[] memory acc,) = _buildES256ReceiptAndAcc(AUTHORITY_LOG_ID, idtimestampBe, es256Pk);
        es256Univocity.publishCheckpoint(AUTHORITY_LOG_ID, 1, acc, receipt, new bytes32[][](0), 0, new bytes32[](0), idtimestampBe);

        assertEq(es256Univocity.authorityLogId(), AUTHORITY_LOG_ID);
        assertTrue(es256Univocity.isLogInitialized(AUTHORITY_LOG_ID));
    }

    /// @notice Build ES256-signed receipt and single-peak accumulator (ADR-0030 leaf formula).
    function _buildES256ReceiptAndAcc(bytes32 logId, bytes8 receiptIdtimestampBe, uint256 es256PrivateKey)
        internal
        view
        returns (bytes memory receipt, bytes32[] memory accumulator, bytes32[] memory inclusionProof)
    {
        ...
        receipt = abi.encodePacked(hex"84", _cborBstr(protected), hex"a0", _cborBstr(payload), _cborBstr(sig));
        accumulator = new bytes32[](1);
        accumulator[0] = sha256(abi.encodePacked(receiptIdtimestampBe, sha256(receipt)));
        inclusionProof = new bytes32[](0);
    }
    ...
    /// @notice Checkpoints with idtimestamp 0 (authority bootstrap) and idtimestamp 1 (target log receipt) both succeed.
    function test_twoCheckpoints_differentIdtimestamps_bothSucceed() public {
        Univocity fresh = new Univocity(BOOTSTRAP, KS256_SIGNER, bytes32(0), bytes32(0));
        bytes32 logId = keccak256("multi-idts");
        (bytes memory receipt0, bytes32[] memory acc1,) = _buildBootstrapReceiptAndAcc(AUTHORITY_LOG_ID, bytes8(0));
        fresh.publishCheckpoint(AUTHORITY_LOG_ID, 1, acc1, receipt0, new bytes32[][](0), 0, new bytes32[](0), bytes8(0));
        assertEq(fresh.authorityLogId(), AUTHORITY_LOG_ID);

        (bytes memory receipt1, bytes32[] memory acc2,) = _buildBootstrapReceiptAndAcc(logId, bytes8(uint64(1)));
        bytes32 leaf0 = sha256(abi.encodePacked(bytes8(0), sha256(receipt0)));
        bytes32 leaf1 = sha256(abi.encodePacked(bytes8(uint64(1)), sha256(receipt1)));
        acc2 = new bytes32[](2);
        acc2[0] = leaf0;
        acc2[1] = leaf1;
        bytes32[][] memory consistencyProof = new bytes32[][](1);
        consistencyProof[0] = new bytes32[](0);
        fresh.publishCheckpoint(AUTHORITY_LOG_ID, 2, acc2, receipt0, consistencyProof, 0, new bytes32[](0), bytes8(0));

        bytes32[] memory targetAcc = new bytes32[](1);
        targetAcc[0] = keccak256("peak");
        fresh.publishCheckpoint(logId, 1, targetAcc, receipt1, new bytes32[][](0), 1, new bytes32[](0), bytes8(uint64(1)));
        assertEq(fresh.getLogState(logId).checkpointCount, 1);
    }
}

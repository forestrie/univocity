...
/// @title BootstrapEs256Root
/// @notice Broadcast root `publishCheckpoint` for an ES256 ImutableUnivocity.
contract BootstrapEs256Root is Script {
    ...
    function run() external {
        ...
        uint256 es256PrivateKey = vm.envUint("ES256_PRIVATE_KEY");
        ...
        bytes32 digest = sha256(sigStruct);
        (bytes32 r, bytes32 s) = vm.signP256(es256PrivateKey, digest);
        s = _ensureLowerS(s);
        ...
        console.log("Bootstrapped ES256 root on:", univocity);
    ...
    function _ensureLowerS(bytes32 s) internal pure returns (bytes32) {
        ...
        unchecked {
            return sv > P256.N / 2 ? bytes32(P256.N - sv) : s;
        }
    }
